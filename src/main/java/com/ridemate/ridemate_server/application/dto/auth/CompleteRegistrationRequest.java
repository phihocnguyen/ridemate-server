...
@NoArgsConstructor
@AllArgsConstructor
@Schema(description = "Complete user registration request (after OTP verification)")
public class CompleteRegistrationRequest {

    @NotBlank(message = "Phone number is required")
    @Pattern(regexp = "^[0-9]{10,11}$", message = "Phone number must be 10-11 digits")
    @Schema(description = "User's phone number (must have verified OTP)", example = "0912345678")
